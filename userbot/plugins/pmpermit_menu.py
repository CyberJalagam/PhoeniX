...
async def _(event):
    ...
    if not pmpermit_sql.is_approved(chat_id):
        ...
        if event.is_private:
         ...
         ONE = ("__Okay. Your request has been registered. Do not spam my master's inbox.You can expect a reply soon.__\n\n"
        ...
        async with borg.conversation(chat) as conv:
         ...
         if y == "1":
             ...
             x = response.text
             if x == "1":
                 await borg.send_message(chat, "`You're welcome here. Please drop your message here, my master will look into it when he cones back.`\n\n **Please don't flood my inbox.**")
                 response = await conv.get_response(chat)
                 if not response.text == "/start":
                     await borg.send_message(chat, LWARN)
                     response = await conv.get_response(chat)
